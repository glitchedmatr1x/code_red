...
This pass is intentionally conservative: it stabilizes the main app layout,
centralizes button creation, and keeps file/archive inspection read-only unless
an explicit export/report action is requested.
...
APP_VERSION = "layout-consolidation-pass-2026-05-05"

# Centralized lane routing. Keep this as the single source of truth for buttons,
# inspector headings, and report grouping.
RESOURCE_LANES: dict[str, tuple[str, ...]] = {
    "Archives": (".rpf", ".zip", ".z01", ".z02", ".z03", ".z04", ".z05", ".z06", ".z07", ".z08", ".z09"),
...
def scan_filesystem(path: Path) -> list[ResourceRecord]:
    ...
    for fp in candidates:
        try:
            ...
            if fp.suffix.lower() in {".z01", ".z02", ".z03", ".z04", ".z05", ".z06", ".z07", ".z08", ".z09"}:
                notes.append("split archive fragment; keep beside matching .zip")
            records.append(
                ResourceRecord(
                    path=str(fp),
                    lane=lane,
...
def scan_zip_members(path: Path, limit: int = 4000) -> list[ResourceRecord]:
    ...
    with zipfile.ZipFile(path, "r") as zf:
        for idx, info in enumerate(zf.infolist()):
            ...
            if info.is_dir():
                continue
            lane = classify_extension(info.filename)
            records.append(
                ResourceRecord(
...
class CodeRedApp:
    ...
    def __init__(self, root: "tk.Tk") -> None:
        ...
        self._build_shell()
        self._set_status("Ready. Open a file or folder to scan Code RED resources.")
    ...
    def _button(self, parent: "tk.Widget", text: str, command: Callable[[], None], *, style: str = "CodeRed.TButton") -> "ttk.Button":
        # One factory for all buttons prevents scattered one-off styling and geometry bugs.
    ...
    def _build_lane_rail(self, body: "ttk.Frame") -> None:
        ...
        hint = "Use lanes instead of loose buttons. All actions stay in the top toolbar."
        ttk.Label(rail, text=hint, style="Muted.TLabel", wraplength=175, justify="left").grid(row=len(RESOURCE_LANES) + 2, column=0, sticky="sew", pady=(12, 0))
    ...
    def _build_workspace(self, body: "ttk.Frame") -> None:
        ...
        widths = {"lane": 100, "extension": 70, "size": 90, "source": 90, "path": 540}
        for col in columns:
    ...
    def scan_archive(self) -> None:
        if self.state.root_path and self.state.root_path.is_file():
            records = scan_zip_members(self.state.root_path)
            if records:
        ...
        if messagebox is not None:
            messagebox.showinfo("Scan Archive", "Open a .zip archive first. RPF parsing remains read-only/staged in this shell.")
    ...
    def load_path(self, path: Path) -> None:
        ...
        if path.is_file() and zipfile.is_zipfile(path):
            self.state.records.extend(scan_zip_members(path))
        self.refresh_records()
    ...
    def refresh_records(self) -> None:
        ...
        records = [rec for rec in self.state.records if selected == "Other" and rec.lane == "Other" or rec.lane == selected]
        ...
        self.summary_label.configure(text="  ".join(f"{lane}:{count}" for lane, count in counts.items() if count))
    ...
    def validate_layout(self) -> None:
        # Runtime geometry check: verifies the three body columns and top toolbar exist.
        ...
        ok = checks["top_actions_actual"] == checks["top_actions_expected"] and checks["lanes_actual"] == checks["lanes_expected"]
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if "--self-test" in argv:
        print(json.dumps({"app": APP_TITLE, "version": APP_VERSION, "lanes": list(RESOURCE_LANES), "actions": TOP_ACTIONS}, indent=2))
        return 0
    if tk is None:
