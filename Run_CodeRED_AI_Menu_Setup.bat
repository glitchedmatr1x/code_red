...
rem It prepares actor data and menu layout, then opens the Build Assistant.
...
if exist "enums.h" (
  echo [1/4] Rebuilding actor enum map from enums.h...
  py -3 tools\codered_actor_enum_tool.py rebuild --source enums.h --replace
  if errorlevel 1 goto :error
) else (
  echo [1/4] enums.h not found. Keeping existing actor enum map.
)
...
echo [3/4] Writing resolved safe roster...
