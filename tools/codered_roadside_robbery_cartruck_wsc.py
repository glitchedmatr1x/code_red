#!/usr/bin/env python3
"""
Code RED Roadside Robbery Car/Truck WSC Patcher

Purpose:
  Patch beat_roadside_robbery.wsc's active wagon/coach vehicle actor IDs from
  1195..1202 to 1193..1194, using the RDR1 trainer-proven actor IDs:
    1193 Truck01
    1194 Car01
...
DEFAULT_OLD_LOW = 1195
DEFAULT_OLD_HIGH = 1202
DEFAULT_NEW_LOW = 1193
DEFAULT_NEW_HIGH = 1194

# RDR1 vehicle actor IDs used by Code RED vehicle patches.
# Keeping this table local prevents direct-ids mode from depending on an
# external config file or an older Direct ID patcher module.
VEHICLE_NAMES = {
    1177: "Cart01",
    1178: "Cart02",
    1179: "Cart003",
...
def zstd_compress(data: bytes, level: int = 12, write_content_size: bool = True) -> bytes:
    """Compress Zstandard using python-zstandard or zstd.exe.

    Some RDR1 WSC resources fit only when the Zstandard frame omits the
    content-size field. The standard decoder still validates these frames.
    """
    try:
        import zstandard as zstd  # type: ignore
        return zstd.ZstdCompressor(level=level, write_content_size=write_content_size).compress(data)
    except TypeError:
        # Older python-zstandard builds may not expose write_content_size.
        import zstandard as zstd  # type: ignore
...
def compress_candidates(payload: bytes) -> list[tuple[str, bytes]]:
    ...
    # Include no-content-size frames because beat_roadside_robbery can miss
    # original payload size by only a few bytes with the default frame header.
...
def fit_compressed_payload(compressed_candidates: list[tuple[str, bytes]], target_size: int, allow_grow: bool = False) -> tuple[str, bytes, dict] | None:
    ...
    if allow_grow and compressed_candidates:
        grown_name, grown_comp = min(compressed_candidates, key=lambda x: len(x[1]))
        attempts.append({"codec": grown_name, "size": len(grown_comp), "fit": "allow-grow-variable-size", "over_by": len(grown_comp) - target_size})
        return grown_name, grown_comp, {
            "fit_mode": "allow-grow-variable-size",
            "codec": grown_name,
            "pad": 0,
            "original_payload_size": target_size,
            "grown_payload_size": len(grown_comp),
            "over_by": len(grown_comp) - target_size,
            "attempts": attempts,
            "warning": "Output WSC is larger than original. This is intended only for RPF tools that can import variable-size replacements."
        }
    return None
...
def patch_index_bounds(payload: bytes, base_id: int, old_low: int, old_high: int, new_low: int, new_high: int, int_format: str = "auto") -> tuple[bytes, list[dict], str]:
    ...
    # For trainer actor IDs, index = actor_id - 1177. So 1195..1202 becomes 6..20;
...
def patch_direct_ids(payload: bytes, old_ids: list[int], target_id: int, int_format: str = "u16be") -> tuple[bytes, list[dict], str]:
    """Patch an explicit set of decoded binary actor IDs to one target actor ID.

    This is the safe Roadside Robbery lane for skipping risky IDs such as
    WagonPrison01 (1197) and WagonGatling01 (1198). It does not patch ASCII
    strings or byte substrings; it only replaces complete integer operands in
    the selected storage format.
    """
    mapping = {int(v): int(target_id) for v in old_ids}
    patched, reps = patch_mapping_by_format(payload, mapping, int_format)
    for r in reps:
        r["safety"] = "explicit decoded binary integer only; no ASCII substring replacement"
        r["old_name"] = VEHICLE_NAMES.get(int(r["old"]), "")
        r["new_name"] = VEHICLE_NAMES.get(int(r["new"]), "")
        r["direct_id_mode"] = True
        r["note"] = "Roadside robbery selective pass: patch chosen wagon/coach IDs only, leave prison/gatling wagon IDs untouched unless explicitly requested."
    return patched, reps, f"direct-ids-{int_format}-to-{target_id}"
...
def analyze(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out) if args.out else Path("logs/roadside_robbery_cartruck_wsc/analyze")
...
def decode_scan(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out) if args.out else Path("logs/roadside_robbery_cartruck_wsc/decode_scan")
    out.mkdir(parents=True, exist_ok=True)
    report = {
        "input": str(path),
        "input_size": len(data),
...
def patch(args: argparse.Namespace) -> int:
    ...
    if args.mode == "direct-ids" and getattr(args, "old_ids", None):
        actor_values = set(int(v) for v in args.old_ids)
    else:
    ...
    elif args.mode == "direct-ids":
        if not getattr(args, "old_ids", None):
            raise SystemExit("direct-ids mode requires --old-ids")
        fmt = "u16be" if args.int_format == "auto" else args.int_format
        patched, replacements, used_format = patch_direct_ids(decoded, args.old_ids, args.target_id, fmt)
    else:
    ...
    report["patch"] = {
        ...
        "before_undesired_old_counts_by_format_value": count_int_values_by_format(decoded, (actor_values if args.mode == "direct-ids" else undesired_values(args.old_low, args.old_high, args.new_low, args.new_high))),
        ...
        "after_undesired_old_counts_by_format_value": count_int_values_by_format(patched, (actor_values if args.mode == "direct-ids" else undesired_values(args.old_low, args.old_high, args.new_low, args.new_high))),
    ...
    fit = fit_compressed_payload(candidates, original_payload_size, allow_grow=getattr(args, "allow_grow", False))
    if fit is None:
...
def status(args: argparse.Namespace) -> int:
    ...
    key, attempts = get_aes_key(root, args.rdr_exe)
    out = {
        "tool": "Code RED Roadside Robbery 1195-1202 to Car/Truck WSC Patcher",
        "version": "2.0-growfit-directids",
        "cwd": str(Path.cwd()),
        "repo_root_guess": str(root),
        "target_default": "imports\\beat_roadside_robbery.wsc",
        "patch_default": "direct-ids can patch selected wagon/coach IDs to Car01; binary-range still available",
        "extra_modes": ["decode-scan", "bounds", "binary-range", "direct-ids", "all-alternating", "ascii-isolated-range", "index-bounds"],
        "cartruck_ids": {"Truck01": 1193, "Car01": 1194},
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Patch beat_roadside_robbery WSC vehicle ID range to Truck01/Car01, with grow-fit fallback.")
    ...
    p.add_argument("--input", default="imports/beat_roadside_robbery.wsc")
    p.add_argument("--out", default="logs/roadside_robbery_cartruck_wsc/analyze")
    ...
    p.add_argument("--input", default="imports/beat_roadside_robbery.wsc")
    p.add_argument("--out", default="logs/roadside_robbery_cartruck_wsc/decode_scan")
    p.add_argument("--old-low", type=int, default=DEFAULT_OLD_LOW)
    p.add_argument("--old-high", type=int, default=DEFAULT_OLD_HIGH)
    p.add_argument("--new-low", type=int, default=DEFAULT_NEW_LOW)
    p.add_argument("--new-high", type=int, default=DEFAULT_NEW_HIGH)
    ...
    p.add_argument("--input", default="imports/beat_roadside_robbery.wsc")
    p.add_argument("--out", default="patches/beat_roadside_robbery_1195_1202_to_1193_1194.wsc")
    p.add_argument("--old-low", type=int, default=DEFAULT_OLD_LOW)
    p.add_argument("--old-high", type=int, default=DEFAULT_OLD_HIGH)
    p.add_argument("--new-low", type=int, default=DEFAULT_NEW_LOW)
    p.add_argument("--new-high", type=int, default=DEFAULT_NEW_HIGH)
    p.add_argument("--mode", choices=["bounds", "binary-range", "direct-ids", "all-alternating", "ascii-isolated-range", "index-bounds"], default="binary-range",
                   help="bounds patches actor-ID bounds 1195->1193 and 1202->1194. binary-range remaps exact decoded binary actor IDs in range, defaulting to u16be. direct-ids patches only explicit --old-ids to --target-id. all-alternating remaps every decoded integer actor ID in range using auto format. ascii-isolated-range only patches standalone 4-digit ASCII tokens 1195..1202. index-bounds patches list indexes 18->16 and 25->17 when scripts store selected vehicle indexes instead of actor IDs.")
    ...
    p.add_argument("--range-map", choices=["alternating", "low", "high"], default="alternating", help="Mapping for ascii-isolated-range and binary-range. alternating maps 1195->1193, 1196->1194, etc.; low maps all to 1193; high maps all to 1194.")
    p.add_argument("--old-ids", nargs="+", type=int, help="direct-ids mode only: explicit old actor IDs to replace, e.g. 1195 1196 1199 1202")
    p.add_argument("--target-id", type=int, default=1194, help="direct-ids mode only: target actor ID, default 1194 Car01")
    p.add_argument("--max-replacements", type=int, default=96, help="Safety cap for replacements before writing. Increase only after reviewing preview CSV.")
    p.add_argument("--allow-many", action="store_true", help="Bypass --max-replacements after reviewing the replacement CSV.")
    ...
    p.add_argument("--allow-grow", action="store_true", help="Allow a structurally valid WSC output larger than the original if exact-size repack is impossible. Use only with an RPF importer that updates file sizes.")
