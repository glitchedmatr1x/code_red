...
TOOL_VERSION = "0.3.0-sector-toggle"
...
# ---------------------------------------------------------------------------
# Sector scanner/toggler lane (v0.3)
# ---------------------------------------------------------------------------
# Proven marker families from Code RED Morning Star sector work:
#   0xF6 = ENABLE_WORLD_SECTOR
#   0xF7 = DISABLE_WORLD_SECTOR (inferred/provisioned; validate in-game before broad use)
#   0xF8 = DISABLE_CHILD_SECTOR
#   0xF9 = ENABLE_CHILD_SECTOR
SECTOR_MARKER_MAP = {
    0xF6: ("world", "enabled", "ENABLE_WORLD_SECTOR"),
    0xF7: ("world", "disabled", "DISABLE_WORLD_SECTOR"),
    0xF8: ("child", "disabled", "DISABLE_CHILD_SECTOR"),
    0xF9: ("child", "enabled", "ENABLE_CHILD_SECTOR"),
}
SECTOR_MARKER_BY_TYPE_STATE = {
    ("world", "enabled"): 0xF6,
    ("world", "disabled"): 0xF7,
    ("child", "disabled"): 0xF8,
    ("child", "enabled"): 0xF9,
}
SECTOR_NAME_RE = re.compile(r"^[A-Za-z0-9_]{3,80}$")
...
def _sector_text_from_slot(slot: bytes, declared_len: int) -> str:
    # Rockstar string literals here usually store len including null terminator.
    raw = slot[:declared_len]
    if b"\x00" in raw:
        raw = raw.split(b"\x00", 1)[0]
    return raw.decode("ascii", errors="ignore")


def find_sector_entries(decoded: bytes, *, include_unknown: bool = False) -> list[SectorEntry]:
    entries: list[SectorEntry] = []
    seen: set[tuple[int, int]] = set()
    for op in range(0, max(0, len(decoded) - 8)):
        if decoded[op] != 0x6F:  # observed string literal opcode before sector names
            continue
        declared_len = decoded[op + 1]
        if declared_len < 4 or declared_len > 96:
            continue
        string_start = op + 2
        declared_end = string_start + declared_len
        if declared_end > len(decoded):
            continue
        name = _sector_text_from_slot(decoded[string_start:declared_end], declared_len)
        if not name or not SECTOR_NAME_RE.match(name):
            continue
        # Sector names in this lane almost always contain '_' or end in xx/x style; keep broad but avoid common short UI tokens.
        if not ("_" in name or name.startswith("dlc") or name.startswith("mp_") or name.endswith("x")):
            continue
        # Marker follows after optional null/zero/space padding: 2C 02 F6/F7/F8/F9.
        search_start = declared_end
        search_end = min(len(decoded), string_start + max(declared_len, 1) + 48)
        marker_prefix = decoded.find(b"\x2c\x02", search_start, search_end)
        if marker_prefix < 0:
            continue
        marker_offset = marker_prefix + 2
        marker = decoded[marker_offset]
        if marker not in SECTOR_MARKER_MAP and not include_unknown:
            continue
        sector_type, state, call = SECTOR_MARKER_MAP.get(marker, ("unknown", "unknown", f"UNKNOWN_0x{marker:02X}"))
        slot_capacity = marker_prefix - string_start
        if slot_capacity <= 0:
            continue
        key = (op, marker_offset)
        if key in seen:
            continue
        seen.add(key)
        entries.append(SectorEntry(
            ...
            string_len_byte_offset=op + 1,
            string_len_byte=declared_len,
            ...
            state=state,
            call=call,
            raw_slot_hex=decoded[string_start:marker_prefix].hex(),
        ))
    return entries


def write_sector_reports(out_dir: Path, input_path: Path, entries: list[SectorEntry], info: dict) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    fields = [
        "index", "name", "sector_type", "state", "call",
        "opcode_offset_hex", "string_offset_hex", "marker_offset_hex",
        "marker_hex", "string_len_byte", "slot_capacity", "raw_slot_hex",
    ]
    with (out_dir / "sector_inventory.csv").open("w", newline="", encoding="utf-8") as f:
        ...
        for e in entries:
            writer.writerow({
                ...
                "state": e.state,
                "call": e.call,
                "opcode_offset_hex": f"0x{e.opcode_offset:X}",
                "string_offset_hex": f"0x{e.string_offset:X}",
                "marker_offset_hex": f"0x{e.marker_offset:X}",
                "marker_hex": f"0x{e.marker:02X}",
                "string_len_byte": e.string_len_byte,
                "slot_capacity": e.slot_capacity,
                "raw_slot_hex": e.raw_slot_hex,
            })
    counts: dict[str, int] = {}
    for e in entries:
        counts[e.call] = counts.get(e.call, 0) + 1
    lines = [
        "# Code RED Sector Scan",
        "",
        f"Input: `{input_path}`",
        f"Mode: `{info.get('mode')}`",
        f"Decoded size: `{info.get('decoded_size', 'n/a')}`",
        f"Sector candidates: `{len(entries)}`",
        "",
        "## Call counts",
        "",
    ]
    ...
    lines += ["", "## First 80 entries", ""]
    for e in entries[:80]:
        lines.append(f"- [{e.index}] `{e.call}(\"{e.name}\")` marker=0x{e.marker:02X} off=0x{e.opcode_offset:X}")
    lines += [
        "",
        "## Notes",
        "",
        "This scanner recognizes the sector literal/marker pattern proven in the Morning Star lane.",
        "It does not rewrite branches or native calls; toggles are same-byte marker changes and same-slot string edits.",
    ]
    (out_dir / "sector_toggle_report.md").write_text("\n".join(lines), encoding="utf-8")
    (out_dir / "file_info.json").write_text(json.dumps(info, indent=2), encoding="utf-8")


def command_sector_scan(args: argparse.Namespace) -> int:
    path = Path(args.input).resolve()
    data = read_bytes(path)
    meta = parse_rsc85_header(data)
    if meta is None:
        raise WorkbenchError("sector-scan currently expects a WSC/RSC85 file.")
    decoded, meta2, _ = decode_rsc85(data)
    entries = find_sector_entries(decoded, include_unknown=args.include_unknown)
    info = {
        "path": str(path),
        "size": len(data),
        "sha256": sha256_bytes(data),
        ...
        "sector_candidates": len(entries),
        "tool_version": TOOL_VERSION,
    }
    out_dir = Path(args.out or f"sector_scan_{path.stem}_{now_stamp()}").resolve()
    write_sector_reports(out_dir, path, entries, info)
    print(f"Sector scan complete: {out_dir}")
    print(f"Sector candidates: {len(entries)}")
    print(f"CSV: {out_dir / 'sector_inventory.csv'}")
    return 0


def _patch_sector_name(buf: bytearray, entry: SectorEntry, new_name: str) -> None:
    new_b = new_name.encode("ascii")
    # One null terminator is part of the literal. Keep any remaining bytes null so MagicRDR decompiles cleanly.
    needed = len(new_b) + 1
    if needed > entry.slot_capacity:
        raise WorkbenchError(
            f"Replacement sector `{new_name}` needs {needed} bytes including null, "
            f"but slot for `{entry.name}` has capacity {entry.slot_capacity}."
        )
    if needed > 255:
        raise WorkbenchError("Sector name literal is too long for one-byte length field.")
    buf[entry.string_len_byte_offset] = needed
    buf[entry.string_offset:entry.string_offset + entry.slot_capacity] = b"\x00" * entry.slot_capacity
    buf[entry.string_offset:entry.string_offset + len(new_b)] = new_b
    buf[entry.string_offset + len(new_b)] = 0


def command_sector_patch(args: argparse.Namespace) -> int:
    path = Path(args.input).resolve()
    out_path = Path(args.out).resolve() if args.out else path.with_name(f"{path.stem}_sectorpatched{path.suffix}")
    original = read_bytes(path)
    decoded, meta, header = decode_rsc85(original)
    entries = find_sector_entries(decoded, include_unknown=True)
    selected: list[SectorEntry] = []
    if args.index is not None:
        for idx in args.index:
            matches = [e for e in entries if e.index == idx]
            if not matches:
                raise WorkbenchError(f"Sector index not found: {idx}")
            selected.extend(matches)
    if args.sector:
        selected.extend([e for e in entries if e.name.lower() == args.sector.lower()])
    # Deduplicate while preserving order.
    dedup: dict[int, SectorEntry] = {}
    for e in selected:
        dedup[e.index] = e
    selected = [dedup[k] for k in sorted(dedup)]
    if not selected:
        raise WorkbenchError("No sector entries selected. Use --sector NAME or --index N. Run sector-scan first.")
    if len(selected) > 1 and not args.all and args.index is None:
        raise WorkbenchError(f"Selected {len(selected)} entries for `{args.sector}`. Add --all or patch by --index.")
    buf = bytearray(decoded)
    changes = []
    for e in selected:
        old_marker = e.marker
        old_name = e.name
        target_type = args.set_type or e.sector_type
        target_state = args.set_state or e.state
        if target_type not in ("world", "child") or target_state not in ("enabled", "disabled"):
            raise WorkbenchError(f"Entry {e.index} has unknown type/state; specify both --set-type and --set-state.")
        new_marker = SECTOR_MARKER_BY_TYPE_STATE[(target_type, target_state)]
        if args.replace_with:
            _patch_sector_name(buf, e, args.replace_with)
        buf[e.marker_offset] = new_marker
        changes.append({
            "index": e.index,
            "old_name": old_name,
            "new_name": args.replace_with or old_name,
            "old_call": e.call,
            "new_call": SECTOR_MARKER_MAP[new_marker][2],
            "old_marker_hex": f"0x{old_marker:02X}",
            "new_marker_hex": f"0x{new_marker:02X}",
            "opcode_offset_hex": f"0x{e.opcode_offset:X}",
            "marker_offset_hex": f"0x{e.marker_offset:X}",
        })
    patched_decoded = bytes(buf)
    rebuilt = repack_rsc85(patched_decoded, meta, header)
    decoded2, _, _ = decode_rsc85(rebuilt)
    if decoded2 != patched_decoded:
        raise WorkbenchError("RSC85 validation failed after sector patch.")
    manifest = {
        "tool_version": TOOL_VERSION,
        "input": str(path),
        "output": str(out_path),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ss = sub.add_parser("sector-scan", help="scan WSC/RSC85 for world/child sector enable-disable entries")
    ss.add_argument("input")
    ss.add_argument("--out")
    ss.add_argument("--include-unknown", action="store_true")
    ss.set_defaults(func=command_sector_scan)

    sp = sub.add_parser("sector-patch", help="toggle or rename selected WSC world/child sector entries")
    sp.add_argument("input")
    sp.add_argument("--sector", help="sector name to select, e.g. esc_villaWall04x")
    sp.add_argument("--index", type=int, action="append", help="sector index from sector_inventory.csv; can be repeated")
    sp.add_argument("--all", action="store_true", help="patch all entries that match --sector")
    sp.add_argument("--set-state", choices=["enabled", "disabled"], help="desired state")
    sp.add_argument("--set-type", choices=["world", "child"], help="desired sector call family")
    sp.add_argument("--replace-with", help="new sector string; must fit in the existing literal slot")
    sp.add_argument("--out")
    sp.add_argument("--dry-run", action="store_true")
    sp.set_defaults(func=command_sector_patch)
    return p
