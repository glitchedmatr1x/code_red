...
Purpose:
  Patch beat_crime_wagonthief.wsc's active vehicle actor-id bounds from
  1183..1197 to 1193..1194, using the RDR1 trainer-proven actor IDs:
    1193 Truck01
    1194 Car01
...
DEFAULT_OLD_LOW = 1183
DEFAULT_OLD_HIGH = 1197
...
def analyze(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out) if args.out else Path("logs/wagonthief_cartruck_wsc/analyze")
...
def decode_scan(args: argparse.Namespace) -> int:
    ...
    out = Path(args.out) if args.out else Path("logs/wagonthief_cartruck_wsc/decode_scan")
    out.mkdir(parents=True, exist_ok=True)
    report = {
        "input": str(path),
        "input_size": len(data),
...
def status(args: argparse.Namespace) -> int:
    ...
    key, attempts = get_aes_key(root, args.rdr_exe)
    out = {
        "tool": "Code RED WagonThief 1183-1197 to Car/Truck WSC Patcher",
        "version": "1.4",
        "cwd": str(Path.cwd()),
        "repo_root_guess": str(root),
        "target_default": "imports\\beat_crime_wagonthief.wsc",
        "patch_default": "safe default remains bounds; use --mode binary-range --int-format u16be for exact decoded binary actor IDs",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Patch beat_crime_wagonthief WSC vehicle ID range to Truck01/Car01.")
    ...
    p.add_argument("--input", default="imports/beat_crime_wagonthief.wsc")
    p.add_argument("--out", default="logs/wagonthief_cartruck_wsc/analyze")
    ...
    p.add_argument("--input", default="imports/beat_crime_wagonthief.wsc")
    p.add_argument("--out", default="logs/wagonthief_cartruck_wsc/decode_scan")
    p.add_argument("--old-low", type=int, default=DEFAULT_OLD_LOW)
    p.add_argument("--old-high", type=int, default=DEFAULT_OLD_HIGH)
    p.add_argument("--new-low", type=int, default=DEFAULT_NEW_LOW)
    p.add_argument("--new-high", type=int, default=DEFAULT_NEW_HIGH)
    ...
    p.add_argument("--input", default="imports/beat_crime_wagonthief.wsc")
    p.add_argument("--out", default="patches/beat_crime_wagonthief_1183_1197_to_1193_1194.wsc")
    p.add_argument("--old-low", type=int, default=DEFAULT_OLD_LOW)
    p.add_argument("--old-high", type=int, default=DEFAULT_OLD_HIGH)
    p.add_argument("--new-low", type=int, default=DEFAULT_NEW_LOW)
    p.add_argument("--new-high", type=int, default=DEFAULT_NEW_HIGH)
    p.add_argument("--mode", choices=["bounds", "binary-range", "all-alternating", "ascii-isolated-range", "index-bounds"], default="bounds",
    ...
    p.add_argument("--int-format", choices=["auto", "u32le", "u16le", "u16be", "u32be", "i16le", "i32le", "u8"], default="auto",
