...
import struct, zlib, subprocess, re, hashlib, shutil
...
def zstd_compress(payload: bytes, level: int=18)->bytes:
    p=subprocess.run(['zstd','-q','-z',f'-{level}','--no-check','--stdout'],input=payload,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True)
    return p.stdout

def compress_like(original_slot: bytes, payload: bytes):
    if original_slot.startswith(b'\x28\xB5\x2F\xFD'):
        # Simple deterministic zstd pass. Avoid multi-candidate stalls.
        levels = [9] if len(payload) > 50000 else [18, 9]
        best_raw=None; best_label=None; first_raw=None; first_label=None
        for level in levels:
            cmd=['zstd','-q','-z',f'-{level}','--no-check','--single-thread','--stdout']
            p=subprocess.run(cmd,input=payload,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True,timeout=60)
            raw=p.stdout
            q=subprocess.run(['zstd','-d','-q','--single-thread','--stdout'],input=raw,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=True,timeout=60)
            if q.stdout!=payload:
                continue
        ...
        if first_raw is not None:
            return first_raw, first_label
        raise RuntimeError('zstd recompress failed')
    for wb in (-15,15,31):
        try:
            zlib.decompress(original_slot,wb)
        except Exception:
            continue
        level=9
        if wb==15: raw=zlib.compress(payload,level)
        else:
            co=zlib.compressobj(level=level,wbits=wb); raw=co.compress(payload)+co.flush()
        if zlib.decompress(raw,wb)==payload:
            return raw, f'zlib-w{wb}-l{level}'
...
def patch_entry(archive_copy_path: Path|str, internal_path: str, payload: bytes):
    ...
    if ent.get('is_resource'):
        raise NotImplementedError('resource entries unsupported here')
    if ent.get('is_compressed'):
        coded, codec=compress_like(slot,payload)
        verify=extract_from_coded(coded, ent)
        assert verify==payload, 'verify failed'
    else:
...
def find_unique_by_name(info, filename: str):
    hs=[rdr_name_hash(filename)]
    if filename.startswith('0x'):
        try: hs.append(int(filename,16))
        except Exception: pass
    matches=[e for e in info['entries'] if e.get('type')=='file' and e.get('name_off') in hs]
    if len(matches)!=1:
        raise KeyError(f'{filename}: {len(matches)} matches')
    return matches[0]

def patch_entry_obj(archive_copy_path: Path|str, ent_selector, payload: bytes):
    archive_copy_path=Path(archive_copy_path)
    info=parse(archive_copy_path, with_debug=False)
    ent = ent_selector(info) if callable(ent_selector) else ent_selector
    # refresh by index
    idx=ent['index']
    ...
    if relocated:
        ...
        with archive_copy_path.open('r+b') as f:
            ...
            if len(coded)<ent['size_in_archive']: f.write(b'\x00'*(ent['size_in_archive']-len(coded)))
        update_metadata(archive_copy_path, info, ent, new_size_in_archive=len(coded), new_total_size=len(payload))
    info2=parse(archive_copy_path, with_debug=False)
    ent2=info2['entries'][idx]
    out=extract(archive_copy_path, ent2)
    if out!=payload: raise AssertionError('re-read mismatch')
    return {'index':idx,'name_off':hex(ent['name_off']),'original_payload_size':len(original),'new_payload_size':len(payload),'original_slot_size':len(slot),'new_slot_size':len(coded),'relocated':relocated,'codec':codec,'old_offset':ent['offset'],'new_offset':ent2['offset'],'sha1':hashlib.sha1(payload).hexdigest()}
