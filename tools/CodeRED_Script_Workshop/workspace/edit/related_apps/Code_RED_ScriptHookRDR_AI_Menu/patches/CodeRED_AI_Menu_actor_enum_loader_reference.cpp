// CodeRED_AI_Menu.cpp
// Conservative ScriptHookRDR in-game menu scaffold with data-driven actor enum support.
//
// Pass: Python/data-file actor enum resolver.
// Purpose: rebuild this ASI once, then add/edit actor enum rows in
// data/codered/actor_enum_map.csv without recompiling for every actor test.
//
// This source resolves ScriptHookRDR exports dynamically with GetProcAddress
// instead of linking against a ScriptHookRDR import .lib. It keeps the build
// simple: cl.exe can produce a .asi DLL using only Windows SDK + this source.
...
static bool parseActorEnumToken(const std::string& raw, int& outValue) {
    ...
    if (text.empty() || text == "?" || text == "null" || text == "NULL" || text == "todo" || text == "TODO") {
        return false;
    }
    int base = 10;
    ...
    if (errno != 0 || end == text.c_str() || trim(end).size() != 0) {
        return false;
    }
...
static std::string enumHex(int actorEnum) {
    char buffer[32] = {};
    snprintf(buffer, sizeof(buffer), "0x%08X", static_cast<unsigned int>(actorEnum));
...
static void loadConfig() {
    ...
    while (std::getline(file, line)) {
        ...
        if (key == "roster") g_rosterPath = value;
        else if (key == "actor_enum_map" || key == "actor_map" || key == "enum_map") g_actorEnumMapPath = value;
        else if (key == "action_plan") g_actionPlanPath = value;
    }
    writeLog("Config loaded: roster=%s actor_enum_map=%s action_plan=%s",
             g_rosterPath.c_str(), g_actorEnumMapPath.c_str(), g_actionPlanPath.c_str());
}
...
static void loadActorEnumMap() {
    ...
    while (std::getline(file, line)) {
        ...
        // Fast one-off syntax: label=1234 or label|0x1234.
        size_t inlineEq = clean.find('=');
        size_t inlinePipe = clean.find('|');
        if (inlineEq != std::string::npos || inlinePipe != std::string::npos) {
            size_t pos = inlineEq;
            if (inlinePipe != std::string::npos && (pos == std::string::npos || inlinePipe < pos)) pos = inlinePipe;
        ...
        if (fields.size() >= 2) {
            std::string key0 = lowerCopy(fields[0]);
            std::string key1 = lowerCopy(fields[1]);
            if ((key0 == "label" || key0 == "name" || key0 == "actor") &&
                (key1 == "actor_enum" || key1 == "enum" || key1 == "value" || key1 == "actorenum")) {
                continue;
            }
        }
        ...
        // Recommended CSV columns: label,actor_enum,category,source,aliases,notes
        if (fields.size() >= 5) {
    ...
    writeLog("Actor enum map loaded: rows=%zu aliases=%zu path=%s",
             g_actorEnumRowsLoaded, g_actorEnumMap.size(), g_actorEnumMapPath.c_str());
}
...
static void loadRoster() {
    ...
    g_dirtyRoster = false;
    writeLog("Roster loaded: count=%zu path=%s", g_roster.size(), g_rosterPath.c_str());
}
...
static int selectedActorEnum() {
    if (g_dirtyActorMap) loadActorEnumMap();
    int actorEnum = 0;
    if (resolveInlineActorEnum(selectedNpcRaw(), actorEnum)) return actorEnum;
    auto found = g_actorEnumMap.find(lowerCopy(selectedNpc()));
    if (found != g_actorEnumMap.end()) return found->second;
    return 0;
}
...
static void writeActionPlan() {
    ...
    if (resolved) {
        g_status = "Queued: " + selectedAction() + " / " + selectedNpc() + " enum " + std::to_string(actorEnum);
        writeLog("Action plan written: action=%s model=%s enum=%d",
                 selectedAction().c_str(), selectedNpc().c_str(), actorEnum);
    } else {
        g_status = "Queued unresolved: add enum for " + selectedNpc();
        writeLog("Action plan written unresolved: action=%s model=%s",
                 selectedAction().c_str(), selectedNpc().c_str());
    }
}
...
static void onKey(DWORD key, WORD, BYTE, BOOL isDown, BOOL, BOOL, BOOL) {
    ...
    if (key == VK_F8 || key == VK_INSERT) {
        ...
        writeLog("Menu toggled: open=%s key=0x%08X", g_menuOpen ? "true" : "false", key);
        return;
    }
...
static void drawMenu() {
    ...
    drawLine(0.035f, 0.170f, "LEFT/RIGHT NPC  |  F5 reload roster + actor_enum_map.csv", 190, 190, 190, 0.25f);

    if (resolved) {
        drawLine(0.035f, 0.198f, "ENUM: " + std::to_string(actorEnum) + " / " + enumHex(actorEnum), 160, 255, 160, 0.25f);
    } else {
        drawLine(0.035f, 0.198f, "ENUM: unresolved - add row in data/codered/actor_enum_map.csv", 255, 170, 90, 0.25f);
    }
