...
- generate a full inline npc_roster.txt as ACTOR_NAME|123 so the in-game menu is not limited
...
VERSION = "1.4.0-roster-pack-pass"
...
DEFAULT_ROSTER_PACK_DIR = Path("data/codered/rosters")
DEFAULT_REPORT = Path("logs/CodeRED_Actor_Enum_Validation_Report.json")
HEADER = ["label", "actor_enum", "category", "source", "aliases", "notes"]
ROSTER_PACKS = {
    "npc_roster_law_gang_companion.txt": "army,law,gang,companion,player_like",
    "npc_roster_lawmen.txt": "army,law",
    "npc_roster_gangs.txt": "gang",
    "npc_roster_mounts_vehicles.txt": "rideable,vehicle",
    "npc_roster_all_spawnable.txt": "actor,army,law,gang,companion,player_like,zombie",
}
...
def canonical_entries_from_rows(rows: Iterable[dict[str, str]]) -> list[tuple[str, int, str]]:
    """Return one ACTOR_* row per positive enum for inline roster generation."""
    chosen: dict[int, tuple[str, int, str]] = {}
    for row in rows:
        raw_value = (row.get("actor_enum") or "").strip()
        try:
        ...
        if not label.upper().startswith("ACTOR_"):
            continue
        category = row.get("category") or category_for(label)
        existing = chosen.get(value)
        if existing is None:
            chosen[value] = (label, value, category)
            continue
        old_label = existing[0]
        # Prefer mixed/canonical ACTOR_ names over all-uppercase/lowercase aliases.
        if old_label.isupper() and not label.isupper():
            chosen[value] = (label, value, category)
        elif old_label.islower() and not label.islower():
            chosen[value] = (label, value, category)
...
def write_inline_roster_from_rows(rows: Iterable[dict[str, str]], output: Path, replace: bool, include_categories: str | None = None) -> Path:
    output.parent.mkdir(parents=True, exist_ok=True)
    if output.exists() and not replace:
        shutil.copy2(output, output.with_suffix(output.suffix + f".bak_{stamp()}"))
    allowed = {x.strip().lower() for x in include_categories.split(",")} if include_categories else None
    entries = canonical_entries_from_rows(rows)
    lines = [
        "# CodeRED full inline actor roster",
        "# Generated by tools/codered_actor_enum_tool.py",
        "# Format: ACTOR_NAME|actor_enum",
        "# The inline value lets the ASI resolve actors without relying on CSV alias parsing.",
        "",
    ]
    count = 0
    for label, value, category in entries:
        if allowed and category.lower() not in allowed:
            continue
        lines.append(f"{label}|{value}  # {category} 0x{value:08X}")
        count += 1
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote inline roster entries: {count}")
    return output
...
def write_roster_pack_from_rows(rows: Iterable[dict[str, str]], output_dir: Path, replace: bool) -> list[Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    written: list[Path] = []
    rows_list = list(rows)
    for filename, categories in ROSTER_PACKS.items():
        output = output_dir / filename
        write_inline_roster_from_rows(rows_list, output, replace, include_categories=categories)
        written.append(output)
    index = output_dir / "README.md"
    if index.exists() and not replace:
        shutil.copy2(index, index.with_suffix(index.suffix + f".bak_{stamp()}"))
    index.write_text(
...
def parser() -> argparse.ArgumentParser:
    ...
    rebuild.add_argument("--write-inline-roster", action="store_true", help="Also replace npc_roster.txt with full ACTOR_NAME|enum roster.")
    rebuild.add_argument("--roster-categories", help="Optional comma list: actor,vehicle,rideable,army,law,gang,companion,player_like,zombie")
    add_replace(rebuild)

    full = sub.add_parser("full-roster", help="Generate full inline npc_roster.txt from existing actor_enum_map.csv.")
    full.add_argument("--categories", help="Optional comma list: actor,vehicle,rideable,army,law,gang,companion,player_like,zombie")
    add_replace(full)

    packs = sub.add_parser("roster-pack", help="Generate editable roster packs for law/gang/companion/vehicle testing.")
    packs.add_argument("--output-dir", type=Path, default=DEFAULT_ROSTER_PACK_DIR)
    add_replace(packs)
...
def main() -> int:
    ...
    if cmd == "seed":
        rows = rows_from_entries(SEED_LABELS, "CodeRED research sanity seed")
    ...
    if cmd == "rebuild":
        ...
        if args.write_inline_roster:
            write_inline_roster_from_rows(rows, args.roster, replace=True, include_categories=args.roster_categories)
        report = validate_roster(args.enum_map, args.roster)
        write_report(report, args.report)
        ...
        for error in report.sanity_errors:
            print(" - " + error)
        return 0 if not report.sanity_errors and report.unresolved_entries == 0 else 2

    if cmd == "full-roster":
        rows = read_map_rows(args.enum_map)
        write_inline_roster_from_rows(rows, args.roster, replace, include_categories=args.categories)
        report = validate_roster(args.enum_map, args.roster)
        write_report(report, args.report)
        print(f"Roster entries:  {report.total_roster_entries}")
        print(f"Resolved:        {report.resolved_entries}")
    ...
    if cmd == "roster-pack":
        rows = read_map_rows(args.enum_map)
        written = write_roster_pack_from_rows(rows, args.output_dir, replace)
        print("Roster pack files:")
        for path in written:
            print(f" - {path}")
        return 0
